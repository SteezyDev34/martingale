...
def main(div_bet_score: object, score_to_start: list[str]) -> bool:
    ...
    try:
        ...
        config.log('Impossible de lire le score du match!', 'warning', True, 3)
        get_bet_score = False
    else:
        config.log('Score en cours : ' + bet_score, 'warning', False, 4, False)
        config.log_clear_line()
        if any(score_ok == bet_score
               for score_ok in
               score_to_start) and len(get_if_icon_ball) > 0:
            get_bet_score = True
            config.log('Score OK : ' + bet_score, 'warning', False, 4, False)
            config.log_clear_line()
