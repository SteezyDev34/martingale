...
def getQTGlobalPerte():
    url = f"{config.api_url}/strategyQT/get_last_perte.php?mtt_recup={config.mtt_recup}"
    try:
        # Envoyer une requête GET à l'URL
        response = requests.get(url)
        # Vérifier que la requête a réussi
...
def getIfQTGlobalPerte():
    url = f"{config.api_url}/strategyQT/get_last_perte.php?infos"
    try:
        # Envoyer une requête GET à l'URL
        response = requests.get(url)
        # Vérifier que la requête a réussi
        ...
        config.log(f'Perte global QT : {str(pertes["perte"])}', 'info', False, 1)
...
def delQTPerte(id):
    url = f"{config.api_url}/strategyQT/del_perte.php?id=" + str(id)
    try:
        # Envoyer une requête GET à l'URL
        response = requests.get(url)
        # Vérifier que la requête a réussi
...
def SendPerteQT(scriptType, perte):
