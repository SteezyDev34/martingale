        ...
        try:
            element = WebDriverWait(driver, 1).until(
                EC.presence_of_element_located((By.XPATH,
                                                '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), "oueur '+str(first_player)+' va gagner le jeu '+str(config.jeu_actuel)+' 40-30")]'))
            )
            ...
            time.sleep(1)
            if tentative_clic ==5:
                config.saveLog("Paris oueur "+str(first_player)+" va gagner le jeu "+str(config.jeu_actuel)+" 40-30 NON TROUVÉ!",config.newmatch)
                ...
                try:
                    ...
                    config.saveLog('Aucun paris 15A TROUVÉ!',config.newmatch)
                    return
                else:
            ...
            try:
                ...
                config.saveLog(f"#E0015\ btn 15A not reachable : {e}",config.newmatch)
            else:
                if len(list_of_bet_type) > 0:
                    ...
                    while not clic and tentative < 5:
                        ...
                        try:
                            ...
                            tentative += 1
                            if tentative ==5:
                                config.saveLog(f"btn 15A not clicable retry : {e}",config.newmatch)
                                return
                            ...
                            while not show_bet_box and tentative_show_bet_box<5:
                                try:
                                    driver.find_element(By.XPATH,
                                                        '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), "oueur '+str(first_player)+' va gagner le jeu '+str(config.jeu_actuel)+' 40-30")]').click()
                                except Exception as e:
                    ...
                    config.saveLog("pas de btn 15 recuperé",config.newmatch)
                    return clic
def GetNextBet4030(driver):
    ...
    while not clic and tentative_clic<30:
        try:
            ...
            time.sleep(1)
            if tentative_clic ==5:
                config.saveLog("Paris Jeu " + str(config.jeu_actuel) + " 15-15 - Oui NON TROUVÉ!",config.newmatch)
                config.saveLog("Vérificattion si autre jeu en cours...",config.newmatch)
                try:
                    element = WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located((By.XPATH,
