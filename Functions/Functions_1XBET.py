...
def remove_match_from_json_file(file_path, match_url, scripttype, timestamp):
    ...
    try:
        ...
        for match in data:
            if not match_removed and 'url' in match and match_url in match['url'] and scripttype in match[
                'scripttype'] and timestamp in match['timestamp']:
                match_removed = True
...
def verification_page_de_match(driver):
    ...
    try:
        ...
        try:
            ...
            print(f"vérificattion page de match impossible!{e}")
...
def retour_section_tps_reglementaire(driver):
    ...
    while temps_reg == 0 and error == 0:
        try:
            ...
            try:
                element = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located((By.CLASS_NAME, 'sub-games-dropdown')))
            except:
                error = 1
                print("error 77")
            else:
...
def selection_des_paris_30_40_du_set(driver, set):
    ...
    while selection == False and tentative < 6:
        try:
            ...
            select_form = driver.find_elements(By.CLASS_NAME, 'sub-games-dropdowny')
            try:
                select_form[0].click()
                time.sleep(3)
            except Exception as e:
...
def recherche_paris_40_30(driver, jeu):
    ...
    try:
        ...
        try:
            ...
            print('Aucun bouton paris 40-15 - Oui TROUVÉ!')
        else:
            list_of_newbet_type = driver.find_elements(By.XPATH,
                                                       '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), "va gagner le Jeu")]')
            if len(list_of_newbet_type) == 1:
