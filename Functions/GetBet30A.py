...
def GetBet30A(driver):
    ...
    while not clic and tentative_clic < 30:
        try:
            ...
            time.sleep(1)
            if tentative_clic == 5:
                config.log("Paris Jeu " + str(config.jeu_actuel) + " 30-30 - Oui NON TROUVÉ!", config.newmatch)
                config.log("Vérificattion si autre jeu en cours...", config.newmatch)
                try:
                    config.log('wait 60 sec to 30A appeear', config.newmatch)
                    element = WebDriverWait(driver, 60).until(
                        EC.presence_of_element_located((By.XPATH,
                                                        '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), " 30-30 - Oui")]'))
                    )
                except Exception as e:
                    config.log('Aucun paris 30A TROUVÉ!', config.newmatch)
                    return
                else:
            ...
            try:
                ...
                config.log(f"#E0015\ btn 30A not reachable : {e}", config.newmatch)
            else:
                if len(list_of_bet_type) > 0:
                    ...
                    while not clic and tentative < 5:
                        ...
                        try:
                            ...
                            tentative += 1
                            if tentative == 5:
                                config.log(f"btn 30A not clicable retry : {e}", config.newmatch)
                                return
                    ...
                    config.log("pas de btn 30 recuperé", config.newmatch)
                    return clic
