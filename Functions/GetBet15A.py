            ...
            element = WebDriverWait(driver, 1).until(
                EC.presence_of_element_located((By.XPATH,
                                                '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), "Jeu ' + str(
                                                    config.jeu_actuel) + ' 15-15 - Oui")]'))
            )
        except Exception as e:
            tentative_clic += 1
            config.log('tentative_clic : ' + str(tentative_clic), config.newmatch)
            time.sleep(1)
            if tentative_clic == 5:
                config.log("Paris Jeu " + str(config.jeu_actuel) + " 15-15 - Oui NON TROUVÉ!", config.newmatch)
                config.log("Vérificattion si autre jeu en cours...", config.newmatch)
                try:
                    ...
                    config.log('Aucun paris 15A TROUVÉ!', config.newmatch)
                    return
                else:
                    list_of_newbet_type = driver.find_elements(By.XPATH,
                                                               '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), " 15-15 - Oui")]')
                    if len(list_of_newbet_type) == 1:
                        config.log('Un seul paris trouvé', config.newmatch)
                        ...
                        config.log('PLUSIEURS JEUX TROUVÉS!', config.newmatch)
                        newbet_type1 = list_of_newbet_type[0].text
                        newbet_type1 = newbet_type1.split(" 15-15 - Oui")
                        game1 = int(newbet_type1[0].split("Jeu ")[1])
                        config.log('PROCHAIN JEU TROUVÉ : ' + str(game1), config.newmatch)
                        newbet_type2 = list_of_newbet_type[1].text
                        config.log(newbet_type2, config.newmatch)
                        newbet_type2 = newbet_type2.split(" 15-15 - Oui")
                        game2 = int(newbet_type2[0].split("Jeu ")[1])
                        config.log('JEU D\'APRÈS TROUVÉ : ' + str(game2), config.newmatch)
                        config.log("vérification ordre de jeu", config.newmatch)
                        if game1 < game2:
                            config.jeu_actuel = game1
                            config.log("Jeu actuel : " + str(config.jeu_actuel), config.newmatch)
                        else:
                            config.jeu_actuel = game2
                            config.log("Jeu actuel : " + str(config.jeu_actuel), config.newmatch)
                    else:
                        config.log('ERROR: try get num game running', config.newmatch)
                        tentative_clic += 1
        else:
            try:
                ...
                config.log(f"#E0015\ btn 15A not reachable : {e}", config.newmatch)
            else:
                if len(list_of_bet_type) > 0:
                    config.log('JEU TROUVÉ! : ' + list_of_bet_type[0].text, config.newmatch)
                    tentative = 0
                    while not clic and tentative < 5:
                        config.log('VERIFICATTION SI CLIQUABLE...', config.newmatch)
                        try:
                            ...
                            tentative += 1
                            if tentative == 5:
                                config.log(f"btn 15A not clicable retry : {e}", config.newmatch)
                                return
                            the_jeu = GetJeuActuel(driver)
                            if config.jeu_actuel != the_jeu:
                                config.log('jeu diff', config.newmatch)
                            ...
                            while not show_bet_box and tentative_show_bet_box < 5:
                                try:
                                    driver.find_element(By.XPATH,
                                                        '//*[@id="allBetsTable"]/div/div[not(contains(@style,"display: none;"))]/div/div[2]/div/span[contains(text(), "Jeu ' + str(
                                                            config.jeu_actuel) + ' 15-15 - Oui")]').click()
                                except Exception as e:
                                    config.log(f"#E0019\nUne erreur est survenue : {e}", config.newmatch)
                                    config.log('CLICK IMPOSSIBLE!', config.newmatch)
                                    ...
                                    try:
                                        ...
                                        config.log('Pas de paris affiché!', config.newmatch)
                                        tentative_show_bet_box += 1
                                        time.sleep(1)
                                    else:
                    ...
                    config.log("pas de btn 15 recuperé", config.newmatch)
                    return clic


def GetNextBet15A(driver):
    print('RECHERCHE DES PROCHAINS PARIS 15 A....')
    GetJeuActuel(driver)
