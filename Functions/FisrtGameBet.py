...
def FirstGameBet(driver):
    config.log('    PREPARATTION PREMIER PARIS', 'title', False)
    bet_40a = False
    tentative = 0
    nextBet = False
    while not bet_40a and not config.error:
        ...
        config.log(f'       Affichage de la liste des paris', 'info', True)

        if not AfficherParis(driver):
            config.error = True
            break
        # On recherche le jeu actuel
        config.log('        liste des pariis affichée', '', True)
        if not GetBet(driver, nextBet):
            tentative = tentative + 1
            if tentative > 5:
                config.log('        error recup jeu #ERR345', 'error', True)
                config.error = True
            continue
        # ON RECHERCHE LES PERTES ET ON CALCUL LA MISE
        GetMise(driver)
        config.log('        Rattrapage : ' + str(config.rattrape_perte), 'error', True)
        tentative_placermise = 0
        validate_bet = False
        config.log('        On place la mise', 'infos', True)
