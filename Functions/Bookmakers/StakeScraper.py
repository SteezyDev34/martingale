...
class StakeScraper:
    ...
    def search_match(self, driver, equipe_1: str, equipe_2: str, sport: str, date: str) -> Optional[str]:
        ...
        try:
                ...
                for btn in btns:
                    try:
                        ...
                        if driver.find_elements(By.CSS_SELECTOR,
                                "input[placeholder='Rechercher sur Stake.com']"):
                            opened = True
                            break
                    except Exception:
                        continue
            ...
            # Essai 2 : tout en JS — trouver ET cliquer le bouton loupe (anonyme dans SECTION)
            if not opened:
            ...
            if not opened:
                log("[Stake] ⚠️ Impossible d'ouvrir la recherche via bouton — tentative directe", "warning")

            # ── 2. Saisir le nom de l'équipe ──────────────────────────────
            _human_wait(0.5, 1.0)
            inp_els = driver.find_elements(By.CSS_SELECTOR,
                "input[placeholder='Rechercher sur Stake.com']")
            if not inp_els:
                log("[Stake] ❌ Barre de recherche non trouvée", "error")
                return None

            search_input = inp_els[0]
            driver.execute_script("""
                arguments[0].focus();
