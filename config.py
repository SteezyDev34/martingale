...
print(f"Vous êtes sur {systeme if systeme in ['Darwin', 'Windows'] else f'Système inconnu : {systeme}'}")
...
def saveLog(txt, prntxt=1, matchname=None):
    """
    ...
    """
    global devMode, newmatch
    
    # Utiliser newmatch si matchname n'est pas fourni
    if matchname is None:
        matchname = newmatch
        
    # Normaliser prntxt à 0 ou 1
    prntxt = 1 if prntxt not in [0, 1] else prntxt
    ...
    nom_de_base = f"{projectPath}/Logs/logScript{scriptType}-{script_num}-{matchname}"
    ...
    try:
        ...
        # Afficher dans la console si demandé ou en mode dev
