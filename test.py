...
for bet in each_bet:
    ...
    data.append(cote)
    mise = secondblock.find_elements(By.CLASS_NAME, 'number-block')[1].find_element(By.CLASS_NAME,
                                                                                    'number-value').text.replace('€',
                                                                                                                 '')
    mise = float(mise) / 10
    data.append(mise)
...
for bets in betslist:
    ...
    driver.find_element(By.CLASS_NAME, 'champ_sport').send_keys('Basket')
