...
<head>
  ...
  <style>
    ...
    /* verte pour haute proba */
    .proba.medium {
      color: #b45309;
    }

    /* orange pour moyenne proba */
    .proba.low {
      color: #b91c1c;
    }

    /* rouge pour faible proba */
    .error {
      color: #b91c1c;
      background: #fee2e2;
...
<body>
  ...
  <main>
    <div class="toolbar">
      ...
      <button id="sortDateDesc">Trier par date décroissante</button>
      <button id="sortDateAsc">Trier par date croissante</button>
      <button id="deleteAll" class="danger">Tout supprimer</button>
  ...
  <script>
    ...
    // Fonction: récupérer les matchs depuis l'API avec tri côté serveur
      ...
      async function chargerMatches(sort = 'desc', sortType = 'probability') {
        effacerErreur();
        ...
        try {
          ...
          // Double sécurité: appliquer un tri client selon le type demandé
          ...
          if (sortType === 'date') {
            ...
            // Tri par probabilité (comportement par défaut)
            data.sort((a, b) => sort === 'asc' ? Number(a.probability) - Number(b.probability) : Number(b.probability) - Number(a.probability));
          }
          
          rendreTable(data);
          const typeLabel = sortType === 'date' ? 'date' : 'proba';
          document.getElementById('status').textContent = `Chargé (${data.length} matchs) — tri ${typeLabel} ${sort.toUpperCase()}`;
        } catch (e) {
    ...
    // Fonction: supprimer tous les matchs
    // Commentaire: Demande confirmation, POST vers delete_all.php, recharge la table.
    async function supprimerTousLesMatchs() {
      effacerErreur();
      if (!confirm('Êtes-vous sûr de vouloir supprimer TOUS les matchs ? Cette action est irréversible.')) {
        return;
      }
      
      const bouton = document.getElementById('deleteAll');
      const oldText = bouton.textContent;
      bouton.textContent = 'Suppression…';
      bouton.disabled = true;
      
      try {
        const resp = await fetch('delete_all.php', {
          method: 'POST',
        ...
        bouton.textContent = 'Supprimé !';
        document.getElementById('status').textContent = `${json.deleted_count} matchs supprimés`;
        
        // Recharger la table pour afficher le résultat vide
        setTimeout(() => {
          chargerMatches('desc', 'probability');
          bouton.textContent = oldText;
    ...
    window.addEventListener('DOMContentLoaded', () => {
      document.getElementById('sortDesc').addEventListener('click', () => chargerMatches('desc', 'probability'));
      document.getElementById('sortAsc').addEventListener('click', () => chargerMatches('asc', 'probability'));
      document.getElementById('sortDateDesc').addEventListener('click', () => chargerMatches('desc', 'date'));
      document.getElementById('sortDateAsc').addEventListener('click', () => chargerMatches('asc', 'date'));
      document.getElementById('deleteAll').addEventListener('click', supprimerTousLesMatchs);
      chargerMatches('desc', 'probability');
    });
  </script>
</body>
