---
description: Placement automatique des paris non traités (Lollybet/Stake via Selenium, 1xBet via l'extension Chrome).
---

Vérifier l'API pour les paris non traités et les placer via process_api_bets.

Setup: config.localhost = 43151, ./venv/bin/python, fenêtre Selenium 11=Stake, 8=Lollybet. Pour 1xBet : le bridge WebSocket doit être démarré (websocket_server.start_bridge()) et l'extension Chrome connectée sur un onglet 1xBet — pas de fenêtre Selenium dédiée.
...
2. Router :
   ...
   - len(matches) > 1 → combiné → place_combined_bet(matches, mise) sur Lollybet
   - sinon → place_best_bet(match) meilleure cote parmi les bookmakers actifs (comparaison inclut 1xBet)
...
5. Si erreur inconnue (sélecteur manquant, nouveau marché) : analyser le DOM, corriger StakeScraper.py/LollybetScraper.py (Selenium) ou chrome_extension/content_scripts/1xbet.js (1xBet), relancer.

Règles :
- Handicap X(0) = "Remboursé si match nul" sur Stake
- Bookmakers actifs : Lollybet ✅ Stake ✅ 1xBet ✅ Winamax ❌ Betclic ❌
